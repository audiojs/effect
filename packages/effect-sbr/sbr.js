...
export default function sbr (data, params = {}) {
	...
	if (!params._src) {
		params._src = { l: 0, b: 0 }           // source band: bandpass fc/2..fc
		params._hp1 = { l: 0, b: 0 }           // output highpass at fc (2 cascaded)
		params._hp2 = { l: 0, b: 0 }
		params._env = 0; params._henv = 0; params._dc = 0
	}
	let fSrc = 2 * sin(PI * Math.min(fc * 0.7, fs / 4) / fs)
	let fCut = 2 * sin(PI * Math.min(fc, fs / 4) / fs)
	let g = 2 + drive * 8
